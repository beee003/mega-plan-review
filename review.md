...
grep -r "TODO\|FIXME\|HACK\|XXX" --include="*.py" --include="*.js" -l
find . -name "*.py" -newer requirements.txt | head -20  # Recently touched files
```
...
* **N+1 queries.** For every new DB query in a loop: is it batched?
...
* **Ecosystem fit.** Does this align with where the Python/JS ecosystem is heading?
